...
class Hander {
  ...
  parseConfig(configPath, iEnv = {}, returnKeys) {
    ...
    if (!iEnv.name && usefulKeys.length) {
      ...
      throw `--name ${iEnv.name} is not the right command, usage: ${Object.keys(
        config
      ).join('|')}`
    ...
    if (!returnKeys || (returnKeys && ~returnKeys.indexOf('alias'))) {
      ...
      // 必要字段
      ;['srcRoot', 'destRoot'].some((key) => {
        if (!config.alias[key]) {
          throw `${chalk.yellow(
            `config.alias.${key}`
          )} is necessary, please check your config: ${chalk.cyan(configPath)}`
        }
      })
      ...
      ;[
        ...
        if (!config.alias[key]) {
          ...
          log(
            ...
            `${chalk.yellow(
              `config.alias.${key}`
            )} is not set, auto fill it: ${chalk.cyan(
              `config.alias.${key} = '${config.alias.destRoot}'`
            )}`
          )
        }
    ...
    if (!returnKeys || (returnKeys && ~returnKeys.indexOf('platform'))) {
      if (!config.platform) {
        ...
        log(
          ...
          `${chalk.yellow(
            'config.platform'
          )} is not exist, build it ${chalk.cyan(
    ...
    if (!returnKeys || (returnKeys && ~returnKeys.indexOf('platform'))) {
      if (!config.name) {
        ...
        log(
          'msg',
          'warn',
          `${chalk.yellow('config.name')} is not exist, build it ${chalk.cyan(
