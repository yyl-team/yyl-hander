...
class Rev {
  constructor({ log, vars }) {
    this.log = log;
    this.vars = vars;
    this.mark = new RevMark({ log, vars });
  ...
  async getRemoteManifest() {
    ...
    if (!disableHash) {
      ...
      const [error, res, content] = await request({
        url: requestUrl,
        headers: {
          'User-Agent': `Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.67 Safari/537.36 YYL/${vars.PKG_VERSION}`
        }
      });
