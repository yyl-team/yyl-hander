  ...
  async varSugar() {
    ...
    jsPaths.forEach((iPath) => {
      ...
      const rCnt = frp.jsPathMatch(iCnt, (iPath, type, q) => {
        ...
        switch (type) {
          case '__url':
            return `__url('${extFn.sugarReplace(iPath, varObj)}')`;

          case '__html':
            htmlUrl = extFn.sugarReplace(iPath, config.alias);
            if (fs.existsSync(htmlUrl)) {
              // step02: html to 字符串
              return extFn.html2string(
                // step01: 格式化 html 里 的路径
                frp.htmlPathMatch(
                  fs.readFileSync(htmlUrl).toString(),
                  (rPath) => {
                    let r = extFn.sugarReplace(rPath, varObj);
                    return r;
                  }
                ),
                q
