...
class Opzer {
  ...
  async initScripts(ctx) {
    ...
    if (typeof ctx === 'string') {
      return await runCMD(ctx, vars.PROJECT_PATH)
    } else if (typeof ctx === 'function') {
      return await ctx({ config, env: iEnv })
    } else {
      return
    }
  }
  ...
  async initBeforeScripts(ctx) {
    const { config, initScripts } = this
    let entry = config.all
    if (ctx === 'watch') {
      entry = config.watch
    }
    if (entry) {
      return await initScripts(entry.beforeScripts)
    }
  }

  /** 初始化 执行后 script */
  async initAfterScript(ctx) {
    const { config, initScripts } = this
    let entry = config.all
    if (ctx === 'watch') {
      entry = config.watch
    ...
    if (entry) {
      return await initScripts(entry.afterScripts)
    }
  }
