...
const util = {
  ...
  path: {
    ...
    join() {
      ...
      const r = path__default['default'].join(...iArgv);
    ...
    relative() {
      ...
      const r = path__default['default'].relative(...iArgv);
      return util.path.handleResult(r, iArgv);
    },
    resolve() {
      let iArgv = util.makeArray(arguments);
      ...
      const r = path__default['default'].resolve(...iArgv);
      return util.path.handleResult(r, iArgv);
    }
  }
};
...
function deepReplace(obj, alias) {
    Object.keys(obj).forEach((key) => {
        ...
        switch (yylUtil.type(obj[curKey])) {
            case 'array':
                toCtx(obj)[curKey] = obj[curKey].map((val) => {
                    if (yylUtil.type(val) === 'string') {
...
class YylHander {
    ...
    formatConfig(option) {
        ...
        if (yylUtil.type(yylConfig.concat) === 'object') {
            if (yylConfig.concat) {
    ...
    openHomePage(op) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (addr) {
                ...
                logger('msg', 'success', source.cyan(addr));
    ...
    saveConfigToServer() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            logger('msg', 'success', `${LANG.CONFIG_SAVED}: ${source.yellow(printPath)}`);
        });
    }
}
